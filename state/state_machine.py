...
class StateMachine[S: Enum, E: Enum, C]:
    ...
    def transition(self, from_state: S | Iterable[S], event: E, to_state: S):
        if not isinstance(from_state, Iterable):
            from_state = (from_state,) # captured in the closure

        def decorator(func: Action[C]) -> Action[C]: # captured in the closure
            for s in from_state:
                self.add_transition(s, event, to_state, func) # registers the transition in the transitions mapping
            return func
